...
class JobOfferService:
    ...
    def find_job_offer(query: dict) -> dict:
        job_offer = db.job_offers.find_one(query)
    ...
    def update_job_offer(query: dict, new_state: str):
        ...
        if JobOfferState.is_readonly(new_state):
            ...
            db.job_applications.update_many(applications_query, {'$set': {'state': JobApplicationState.REJECTED}})
            update_object['dateClosed'] = datetime.now()

        db.job_offers.update_one(query, {'$set': update_object})
        # todo send event to other microservices

    @staticmethod
    def create_job_offer(dto: CreateJobOfferDto) -> dict:
        job_offer = JobOffer.from_dto(dto)
        # job_offer.poster = todo get user info from user service
        fake_poster = User(dto.poster_id, "tests@email.com", datetime.now(), '+420 123 456 789')
